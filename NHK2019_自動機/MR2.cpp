...
double leglength[3] = { 10,143.01*1.5,297.95/1.5 };		// 脚の長さ[mm]、{角柱の一辺の長さ,脚の上下の関節から膝の関節までの長さ、膝の関節から足先までの長さ}
...
double slopepos[3] = { 0,1600,0 };				// スロープの位置
double slopecolor[3] = { 1.0,0.85,0.7 };		// スロープの色

double sandpos[3] = { 0,800,0 };				// サンドの位置
...
static void nearCallback(void *data, dGeomID o1, dGeomID o2)
{
	...
	if (n > 0) {
		for (int i = 0; i < n; i++) {
			...
			contact[i].surface.slip1 = 0.02/2.5;	// FDS摩擦係数
			contact[i].surface.slip2 = 0.02/2.5;	// FDS摩擦係数
